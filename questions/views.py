...
def question_list(request):
    ...
    if "searchbar" in request.GET:
        search_text = request.GET["searchbar"]
        question_list = Question.objects.filter(title__icontains=search_text).order_by(
            "-created_at"
        )
    else:
...
def question_details(request, slug):
    ...
    # return render(
    #     request, "QDetails.html", {"question": question, "answer_list": answer_list}
    # )
...
def list_information(request):
    ...
    return render(
        request, "list_information.html", {"questions": questions, "answers": answers}
    )
